...
export function ValidationProduction() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/gdim/production/${poste}`)
      .then(res => res.json())
      .then((data: any[]) => {
        setOfs(data.map(item => ({
          ...
          commentaire_planif: item.commentaire_planif || '',
        })))
      })
      .catch(err => {
  ...
  const validateOF = useCallback(async (id: string) => {
    ...
    if (!of || !poste) return
    try {
      const res = await fetch(`http://localhost:5000/api/gdim/production/statut/${poste}/${of.n_ordre}`, {
        method: 'PUT',
...
export const Route = createFileRoute('/UAPS/G-DIM/Validation/production')({
