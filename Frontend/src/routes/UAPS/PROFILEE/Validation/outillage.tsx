...
const POSTES = ["CHR01", "VCF-A", "CIN-A"];
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/api/profilee/outils/${poste}`)
      .then(res => res.json())
      .then(data => {
  ...
  const validateTool = useCallback(async (id: string) => {
    ...
    try {
      await fetch(`http://localhost:5000/api/profilee/outils/statut/${poste}/${tool.n_ordre}`, {
        method: "PUT",
...
export const Route = createFileRoute('/UAPS/PROFILEE/Validation/outillage')({
