...
export async function fetchPlanning(poste: string, apiBase: string) {
  const response = await fetch(`http://localhost:5000/api/${apiBase}/planning/${poste}`);
...
export async function savePlanning(poste: string, fileName: string, data: any[], apiBase: string) {
  const res = await fetch(`http://localhost:5000/api/${apiBase}/planning/${poste}`, {
    method: 'PUT',
...
export function PlanningEditor({
  ...
  useEffect(() => {
    ...
    fetchPlanning(poste, apiBase)
      .then(({ data, headers, fileName, version }) => {
        setData(data);
        setHeaders(headers);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      <NavigationMenu />
      <div className="p-8">
        <h1 className="text-2xl font-bold mb-6 text-black">Modifier le planning</h1>
