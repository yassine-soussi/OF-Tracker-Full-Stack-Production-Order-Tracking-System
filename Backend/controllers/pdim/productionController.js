...
const getProductionByPoste = async (req, res) => {
  ...
  try {
    ...
    const [matiereRes, outilRes, ofRes] = await Promise.all([
      pool.query(`SELECT n_ordre, statut FROM statut_matiere_pdim WHERE poste = $1`, [poste]),
      pool.query(`SELECT n_ordre, statut FROM statut_outils_pdim WHERE poste = $1`, [poste]),
      pool.query(`SELECT n_ordre, statut_of, duree, ordre FROM production_statut_pdim WHERE poste = $1 ORDER BY ordre ASC`, [poste]),
    ]);
