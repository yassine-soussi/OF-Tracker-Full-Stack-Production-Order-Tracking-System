...
const getHistoriqueComplet = async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT * FROM historique_complet_profilee
...
const getOutilsStatut = async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT *
      FROM statut_outils_profilee
      ORDER BY poste, n_ordre, ordre;
    `);
    res.json(rows);
  } catch (error) {
    console.error("Erreur récupération statut_outils_profilee:", error);
