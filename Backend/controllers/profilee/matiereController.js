...
const getMatieresByPoste = async (req, res) => {
  ...
  try {
    ...
    const statutRes = await pool.query(
      `SELECT n_ordre, ordre, statut, besoin_machine FROM statut_matiere_profilee WHERE poste = $1`,
      [poste]
    );
...
const updateStatutMatiere = async (req, res) => {
  ...
  try {
    ...
    // ➤ 1. Upsert dans statut_matiere_profilee
    if (statut === 'ready' || statut === 'pending') {
      await client.query(
        `INSERT INTO statut_matiere_profilee 
      ...
      // ➤ 2. Insertion dans notifications_profilee
      await client.query(`
