...
const getProductionByPoste = async (req, res) => {
  ...
  try {
    ...
    const [matiereRes, outilRes, ofRes] = await Promise.all([
      pool.query(`SELECT n_ordre, statut FROM statut_matiere_gdim WHERE poste = $1`, [poste]),
      pool.query(`SELECT n_ordre, statut FROM statut_outils_gdim WHERE poste = $1`, [poste]),
      pool.query(`SELECT n_ordre, statut_of, duree, ordre FROM production_statut_gdim WHERE poste = $1 ORDER BY ordre ASC`, [poste]),
    ]);
...
const updateStatutOf = async (req, res) => {
  ...
  try {
    ...
    const currentRes = await client.query(
      `SELECT statut_of FROM production_statut_gdim WHERE poste = $1 AND n_ordre = $2 AND ordre = $3`,
      [poste, n_ordre, ordre]
    );
    ...
    if (statut_of === 'ready') {
      await client.query(`
        INSERT INTO production_statut_gdim 
