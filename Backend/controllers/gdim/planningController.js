...
const createOrUpdatePlanning = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO planning_gdim (poste, fileName, data, importDate, version)
       VALUES ($1, $2, $3, NOW(), 1)
       ON CONFLICT (poste)
       DO UPDATE SET fileName = $2, data = $3, importDate = NOW(), version = planning_gdim.version + 1
       RETURNING version`,
...
const getPlanning = async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT fileName, data, version FROM planning_gdim WHERE poste = $1`,
      [poste]
    );
    ...
    if (rows.length > 0) {
      ...
      res.status(404).json({ message: "Aucun planning_gdim trouvé pour ce poste" });
    }
  } catch (err) {
    console.error(err);
...
const deletePlanning = async (req, res) => {
  const { poste } = req.params;
  try {
    await pool.query(`DELETE FROM planning_gdim WHERE poste = $1`, [poste]);
    res.json({ success: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Erreur lors de la suppression du planning_gdim" });
  }
};
...
const updatePlanning = async (req, res) => {
  ...
  try {
    const existing = await pool.query(`SELECT version FROM planning_gdim WHERE poste = $1`, [poste]);
    ...
    res.json({ success: true, message: `Planning_gdim enregistré pour ${poste}`, version });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Erreur lors de la sauvegarde du planning_gdim" });
  }
};
